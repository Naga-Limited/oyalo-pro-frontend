...
import {getUploadedSwiggy} from '../../../@app/master/masterSlice';
import {useDispatch, useSelector } from 'react-redux';
import { Row, Col, Form, DatePicker,Card} from "antd";
import { useForm, Controller } from "react-hook-form";


export default function swiggyDetails({setTopTitle}) {
  setTopTitle('Swiggy Details');
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { control } = useForm();
  const handleDownload = () => {
    window.open(
      process.env.REACT_APP_API_BASE_URL + "download-swiggy",
      "_blank"
    );
  };
  ...
  const handleFormSubmit = () => {
    if (Daterange.length != 0) {
      ...
      dispatch(
        getUploadedSwiggy({
          path: "get-Uploaded-Swiggy-Trans",
  ...
  const {
    gettingUploadedSwiggy,
    getUploadedSwiggyResponse: {data: dataSource}
  } = useSelector((state) => {
    return state.master;
  });

  const onClickUpdateCsv = () => {
    navigate("/swiggyDetails/csvUpdate", {
      state: {},
    });
  };
